...
staff_lower, staff_upper, staff_thresh = 80, 150, 0.80 #change thresh to 0.7 -- okay till 0.8
sharp_lower, sharp_upper, sharp_thresh = 100, 150, 0.70 #okay till 0.7
flat_lower, flat_upper, flat_thresh = 75, 150, 0.80
quarter_lower, quarter_upper, quarter_thresh = 80, 150, 0.80 #okay till 0.85
half_lower, half_upper, half_thresh = 75, 150, 0.75  #okay till 0.75
whole_lower, whole_upper, whole_thresh = 50, 100, 0.80
...
def locate_images_elem(img, templates, start, stop, threshold, boxes):
    ...
    for box in boxes:
        ...
        one_box_img = cv2.copyMakeBorder(one_box_img, ypadd, ypadd, 0, 0, cv2.BORDER_CONSTANT, value=255)  # zero padding
        locations, scale = fit(one_box_img, templates, start, stop, threshold)
        for i in range(len(templates)):
            w, h = templates[i].shape[::-1]
            w *= scale
            h *= scale
            img_locations.append([Rectangle(box.x + pt[0], box.y + pt[1] - ypadd, w, h) for pt in zip(*locations[i][::-1])])
...
if __name__ == "__main__":
    #img_file = "5.jpg"
    img_file = input("Please Choose Image(Ex: '8.jpg'): ")
    scan(img_file)
    img_file = "Sheets\\sheetext.jpg"
    img = cv2.imread(img_file, 0)
    ratio = int(img.shape[0] / 600)
    img_gray = img
    img = cv2.cvtColor(img_gray,cv2.COLOR_GRAY2RGB)
    #ret,img_gray = cv2.threshold(img_gray,127,255,cv2.THRESH_BINARY) #adaptive thresh
    img_width, img_height = img_gray.shape[::-1]

    print("Matching staff image...")
    staff_recs, best_scale = locate_images(img_gray, staff_imgs, staff_lower, staff_upper, staff_thresh)
    ...
    sharp_recs = locate_images_elem(img_gray, sharp_imgs, sharp_lower, sharp_upper, sharp_thresh,staff_boxes)
    ...
    quarter_recs = locate_images_elem(img_gray, quarter_imgs, quarter_lower, quarter_upper, quarter_thresh,staff_boxes)
    ...
    half_recs = locate_images_elem(img_gray, half_imgs, half_lower, half_upper, half_thresh,staff_boxes)
    ...
    whole_recs = locate_images_elem(img_gray, whole_imgs, whole_lower, whole_upper, whole_thresh,staff_boxes)
    ...
    for box in staff_boxes:
        staff_sharps = [Note(r, "sharp", box,img = img_gray,staff_img=staff_imgs,best_scale=best_scale)
            for r in sharp_recs if abs(r.middle[1] - box.middle[1]) < box.h*5.0/8.0]
        staff_flats = [Note(r, "flat", box,img = img_gray,staff_img=staff_imgs,best_scale=best_scale)
            for r in flat_recs if abs(r.middle[1] - box.middle[1]) < box.h*5.0/8.0]
        quarter_notes = [Note(r, "4,8", box, staff_sharps, staff_flats,img = img_gray,staff_img=staff_imgs,best_scale=best_scale)
            for r in quarter_recs if abs(r.middle[1] - box.middle[1]) < box.h*5.0/8.0]
        half_notes = [Note(r, "2", box, staff_sharps, staff_flats,img = img_gray,staff_img=staff_imgs,best_scale=best_scale)
            for r in half_recs if abs(r.middle[1] - box.middle[1]) < box.h*5.0/8.0]
        whole_notes = [Note(r, "1", box, staff_sharps, staff_flats,img = img_gray,staff_img=staff_imgs,best_scale=best_scale)
